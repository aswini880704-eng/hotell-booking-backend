...
return;
    ...
    return (
        <nav className={`fixed top-0 left-0 w-full flex items-center justify-between
         px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ?
          "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
            ...
            <div className="hidden md:flex items-center gap-4 lg:gap-8">
                {navLinks.map((link, i) => (
                    <a key={i} href={link.path} className={`group flex flex-col gap-0.5 ${isScrolled ? 
            ...
            <div className="hidden md:flex items-center gap-4">
                <img src= {assets.searchIcon} alt="search" className={`$ {isScrolled && invert } h-7 transection-all duration-500`} />
              ...
              onClick={() => navigate('/my-bookings')}/>
                ...
                transition-all duration-500 ${isScrolled ? "text-white" : "text-black"}`}>
                    Login
                </button>)
                }
              <button onClick={openSignIn} className={`bg-black text-white px-8 py-2.5 rounded-full ml-4
